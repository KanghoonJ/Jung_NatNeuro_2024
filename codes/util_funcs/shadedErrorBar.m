...
% 'patchSaturation'- [0.5 by default] The saturation of the patch color.
%
%
%
% Outputs
...
params.addParameter('lineProps', '-b', @(x) ischar(x) | iscell(x));
params.addParameter('transparent', true, @(x) islogical(x) || x==0 || x==1);
params.addParameter('patchSaturation', 0.5, @(x) isnumeric(x) && x>=0 && x<=1);
