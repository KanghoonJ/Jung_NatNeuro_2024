...
data_folder = 'C:\Users\jungk\Google Drive\Github\Jung_NatNeuro_2024\codes\Fig6\';
...
xrange = [-0.25 10]; % Define x-axis range for plots
...
for nFolder = 1:Num_Folders
    ...
    % Generate the main figures with consistent settings
    fig1 = figure(1), 
    ...
    % Identify event onset and offset frames
    Event_onset_Frame = find(db_Event.ROI_Track_T.ROI_Timestamp_adjusted>=0,1);
    Event_offset_Frame = min([find(db_Event.ROI_Track_T.ROI_Timestamp_adjusted<10,1,'last'), find(db_Event.ROI_Track_T.Dist2Shelter<=5,1)]);
    ROI_Frames = [Event_onset_Frame:Event_offset_Frame];

    % Store normalized distance to shelter data
    ...
    % Handle flight-only and full data analysis
    if(Flight_only)
...
% Retrieve current figure position
pos = get(gcf, 'Position') %// gives x left, y bottom, width, height

%% Plot Average Cross-Correlation for Normalized Distance
...
% Save averaged data to an Excel file
