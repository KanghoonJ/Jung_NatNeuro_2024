...
%% Shelter preference
%% Data analysis 4-1) Shelter_preference
Field_name = ['GP_shelter_preference'];
disp(['Analyzing 4 ' Field_name]);
tic
[db] = GP_shelter_preference_func(db);
drawnow
cd(db.DAQ.Subject_Data_folder)
...
% Event_on_off = db.Matched_data.Matched_Behavior_data.Shelter_in_out_time;
...
yl = ylim;
% Plot_Patch(Patch_x,Patch_y, Event_on_off); hold on;
% Draw Stim time
...
drawnow
xlim(roi_x)
%         ylim([0 max(Patch_y)+5])

ax2 = subplot(9,1,3:9)
ncells = size(DFoF,2);
%         imagesc(Patch_x,1:ncells,zscore_DFoF(:,[Shelter_Neurons; Nonspecific_Neurons; Outside_Neurons]')'); hold on;
...
% imagesc(Patch_x,1:ncells,zscore_DFoF'); hold on;
caxis([-2 2])
colormap(viridis);
set(gca,'YDir','normal','YTick',[1 numel(Shelter_Neurons) numel(Shelter_Neurons)+numel(Nonspecific_Neurons) ncells],'box','off', 'fontsize',24,'FontWeight','bold','tickdir','out');
xlabel('Time (s)');    
ylabel('Neuron');
xlim(roi_x)        
linkaxes([ax1, ax2],'x')
